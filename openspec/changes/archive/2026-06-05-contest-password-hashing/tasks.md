...
- [x] 6.1 手动验证：创建比赛带密码，数据库中存储为 bcrypt 哈希而非明文
- [x] 6.2 手动验证：使用正确密码报名验证成功，错误密码被拒绝
- [x] 6.3 手动验证：管理员 API 响应中不包含 `accessPassword` 字段
- [x] 6.4 手动验证：迁移脚本对已有明文密码执行哈希，重复执行不重复哈希
