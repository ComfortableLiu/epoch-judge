...
- [x] 6.1 手动验证：简单正则可检测的模式（如直接 `require('fs')`）仍然被正则快速拦截
- [x] 6.2 手动验证：变量别名绕过（如 `const cp = require('child_process'); cp.exec('...')`）被 AST 扫描检测
- [x] 6.3 手动验证：动态执行（`eval()`、`new Function()`）被检测
- [x] 6.4 手动验证：全局对象访问（`process.exit()`）被检测
- [x] 6.5 手动验证：合法代码（纯算法解题）不被误报
- [x] 6.6 手动验证：Python/Java/C/C++ 代码走正则路径，不触发 AST 扫描
